...
static int stringToNum(char *s, char *name)
{
    ...
    treeNode node;
    long position = 1L;
    fseek(fp, position*sizeof(treeNode), SEEK_SET);
    fread(&node, sizeof(treeNode), 1, fp);
    while (*s != '\0')
    {
        while (node.content != *s)
        {
            if (node.content == 0)
            {
                goto out;
...
status checkData(char *input, int k, char *tree)
{
    int goal = stringToNum(input, tree);
